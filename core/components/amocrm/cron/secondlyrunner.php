...
if (!$amoBase = $modx->getService(
    'amocrm',
    'amoCRM',
    $modx->getOption(
        'amocrm_core_path',
        null,
        $modx->getOption('core_path') . 'components/amocrm/'
    ) . 'model/amocrm/',
    []
)
) {
...
if (!$simpleQueue = $modx->getService(
    'simplequeue',
    'simpleQueue',
    $modx->getOption(
        'simplequeue_core_path',
        null,
        $modx->getOption('core_path') . 'components/simplequeue/'
    ) . 'model/simplequeue/',
    []
)
) {
...
$processing = $modx->getCount(sqMessage::class, [
    'processing' => true
]);
if ($processing > 0) {
    return;
...
$q->where(['service' => amoCRMTools::SQ_SERVICE, 'processed' => 0, 'status:<' => 5]);
$q->sortby('id');
/** @var sqMessage $tasks */
if ($task = $modx->getObject('sqMessage', $q)) {
    ...
    try {
        ...
        switch ($task->get('action')) {
            ...
            case amoCRMTools::SQ_ACTION_ADD_CONTACT:
                $processed = (bool)$amo->addContact(
                    $properties['userData'],
                    $properties['modUserId'],
                    $properties['leads']
                );
                break;
            ...
            case amoCRMTools::SQ_ACTION_CHANGE_ORDER_STATUS:
                ...
                if ($msOrder = $modx->getObject('msOrder', $subject)) {
                    if ($modx->getOption('amocrm_update_order_on_change_status')) {
                        ...
                        $processed = (bool)$amo->changeOrderStatusInAmo(
                            $properties['ms2OrderId'],
    ...
    if (!empty($processed)) {
        $response = $modx->runProcessor(
            'message/close',
            ['id' => $task->get('id')],
            ['processors_path' => $simpleQueue->config['processorsPath']]
        );
        $modx->error->message = null; // Обнуляем переменную
    } else {
