...
class MsMCValidator {
    ...
    public $modx = null;
    /**
     * @var MsMC $msmc A reference to the MsMC instance.
     * @access public
     */
    public $msmc = null;
     ...
     *
     * @param MsMC &$msmc A reference to the MsMC class instance.
     * @param array $config Optional. An array of configuration parameters.
     * @return MsMCValidator
     */
    function __construct(MsMC &$msmc,array $config = array()) {
        ...
        $this->config = array_merge(array(
            ...
            'customValidators' => !empty($this->msmc->config['customValidators']) ? explode(',',$this->msmc->config['customValidators']) : array(),
        ),$config);
    }
     ...
     *
     * @access public
     * @param MsMCDictionary $dictionary The fields to validate.
     * @param string $validationFields
     * @return array An array of field name => value pairs.
     */
    public function validateFields(MsMCDictionary $dictionary,$validationFields = '') {
     ...
     * either be a method name of MsMCValidator or a Snippet name.
    ...
    public function validate($key,$value,$type = '') {
        ...
        if (method_exists($this,$type) && !in_array($type,$invNames)) {
            ...
            if ($snippet) {
                ...
                $props = array_merge($this->msmc->config,array(
                    'key' => $key,
                    'value' => $value,
                    'param' => $param,
                    'type' => $type,
                ...
                /* no validator found */
                $this->modx->log(modX::LOG_LEVEL_ERROR,'[MsMC] Could not find validator "'.$type.'" for field "'.$key.'".');
                $validated = true;
            }
        } else {
            $this->modx->log(modX::LOG_LEVEL_INFO,'[MsMC] Validator "'.$type.'" for field "'.$key.'" was not specified in the customValidators property.');
            $validated = true;
        }
    ...
    public function addError($key,$value) {
        $errTpl = $this->modx->getOption('errTpl',$this->msmc->config,'<span class="error">[[+error]]</span>');
    ...
    protected function _getErrorMessage($field,$parameter,$lexiconKey,array $properties = array()) {
        if (!empty($this->msmc->config[$field.'.'.$parameter])) {
            $message = $this->msmc->config[$field.'.'.$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else if (!empty($this->msmc->config[$parameter])) {
            $message = $this->msmc->config[$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else {
