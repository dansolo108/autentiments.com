...
if (!class_exists('PayPalCustom')) {
    exit('Error: could not load payment class "PayPalCustom".');
}
/** @var msOrder $order */
$order = $modx->newObject('msOrder');
/** @var msPaymentInterface|PayPalCustom $handler */
$handler = new PayPalCustom($order);
...
if (!is_array($response)) {
    $modx->log(modX::LOG_LEVEL_ERROR,
        '[miniShop2] Error on receive details of PayPalCustom operation: ' . $response . '; ' . print_r($_GET, 1)
    );
    ...
    $modx->log(modX::LOG_LEVEL_ERROR,
        '[miniShop2] Error on receive details of PayPalCustom operation: ' . print_r($response, 1) . '; ' . print_r($_GET, 1)
    );
}
