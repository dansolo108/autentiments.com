Polylang.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper polylang-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        autosave: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('polylang-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('polylang-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('polylang-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                   // cls.push('polylang-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    Polylang.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(Polylang.grid.Default, MODx.grid.Grid, {
    ...
    getColumns: function (config) {
        return [{
            ...
            header: _('polylang.header_actions'),
            dataIndex: 'actions',
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = Polylang.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('polylang-grid-default', Polylang.grid.Default);