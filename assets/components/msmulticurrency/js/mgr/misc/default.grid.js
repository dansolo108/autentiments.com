MsMC.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper msmulticurrency-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        autosave: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('msmulticurrency-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('msmulticurrency-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('msmulticurrency-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('msmulticurrency-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    MsMC.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(MsMC.grid.Default, MODx.grid.Grid, {
    ...
    getSearchField: function (width) {
        return {
            xtype: 'multicurrency-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = MsMC.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('msmc-grid-default', MsMC.grid.Default);