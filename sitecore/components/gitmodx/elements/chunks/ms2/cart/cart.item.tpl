...
<li class="au-cart__card {if $template == 11}au-order__card{/if}" id="{$key}" data-product="{$product_id}" >
        ...
        <div class="au-cart__row">
            ...
            {if $template != 11}
                ...
                <form method="post" class="au-cart__card-count-form ms2_form" role="form">
                    ...
                    <span>
                        <input class="au-cart__card-count" type="number" name="count" value="{$count}" {if isset($remains)}max="{$remains}"{else}max="999"{/if}>
                    </span>
    ...
    <a class="au-full-link" href="{$product_id|url}?{$size?'size='~$size~'&':''}{$color?'color='~$color:''}" aria-label="{$pagetitle}"></a>
</li>