...
class PayPalCustom extends msPaymentHandler implements msPaymentInterface
{
    ...
    function __construct(xPDOObject $object, $config = array())
    {
        ...
        $paymentUrl = $siteUrl . substr($assetsUrl, 1) . 'payment/paypalcustom.php';
    ...
    public function send(msOrder $order)
    {
        ...
        $cid = $properties['msmc']['id'];
        $currencyCode = $cid == 1 ? 'RUB' : 'EUR';
        
        $msmcCartCost = $this->modx->runSnippet('msMultiCurrencyPrice', [
            'price' => $order->get('cart_cost'),
            'cid' => $cid,
        ]);
        $msmcDeliveryCost = $this->modx->runSnippet('msMultiCurrencyPrice', [
            'price' => $order->get('delivery_cost'),
            'cid' => $cid,
        ]);
        $msmcCost = $this->modx->runSnippet('msMultiCurrencyPrice', [
            'price' => $order->get('cost'),
            'cid' => $cid,
        ]);
        $msmcCartCost = str_replace([',',' '], ['.',''], $msmcCartCost);
        $msmcDeliveryCost = str_replace([',',' '], ['.',''], $msmcDeliveryCost);
        $msmcCost = str_replace([',',' '], ['.',''], $msmcCost);
        // восстанавливаем соответствие суммы корзины с доставкой и стоимости заказа
        $subtotal = $msmcCartCost + $msmcDeliveryCost;
        if ($subtotal > $msmcCost) {
            $msmcDeliveryCost = $msmcDeliveryCost - ($subtotal - $msmcCost);
        }
        $params = array(
            'METHOD' => 'SetExpressCheckout',
            'PAYMENTREQUEST_0_CURRENCYCODE' => $currencyCode ?: $this->config['currency'],
            'PAYMENTREQUEST_0_ITEMAMT' => $msmcCartCost,
            'PAYMENTREQUEST_0_SHIPPINGAMT' => $msmcDeliveryCost,
            'PAYMENTREQUEST_0_AMT' => $msmcCost,
        ...
        if ($this->modx->getOption('ms2_payment_paypal_order_details', null, true)) {
            ...
            foreach ($products as $item) {
                ...
                $msmcPrice = $this->modx->runSnippet('msMultiCurrencyPrice', [
                ...
                $params['L_PAYMENTREQUEST_0_AMT' . $i] = str_replace([',',' '], ['.',''], $msmcPrice);
    ...
    public function receive(msOrder $order, $params = array())
    {
        if (!empty($params['PAYERID'])) {
            $currencyCode = $params['PAYMENTREQUEST_0_CURRENCYCODE']?: $this->config['currency'];
            $cid = 1;
            if ($currencyCode == 'USD') $cid = 5;
            $params = array(
