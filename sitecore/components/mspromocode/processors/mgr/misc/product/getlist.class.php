...
class msProductGetListProcessor extends modObjectGetListProcessor
{
    ...
    public function prepareQueryBeforeCount(xPDOQuery $c)
    {
        ...
        if ($this->getProperty('combo')) {
            $c->select('msProduct.id,msProduct.pagetitle,msProduct.context_key,msProduct.published');
        } else {
        ...
        if ($query = $this->getProperty('query', null)) {
            $queryWhere = array(
                'msProduct.id' => $query, 'OR:msProduct.pagetitle:LIKE' => '%'.$query.'%', 'OR:description:LIKE' => '%'.$query.'%', 'OR:introtext:LIKE' => '%'.$query.'%', 'OR:Data.article:LIKE' => '%'.$query.'%', 'OR:Data.vendor:LIKE' => '%'.$query.'%', 'OR:Data.made_in:LIKE' => '%'.$query.'%', 'OR:Vendor.name:LIKE' => '%'.$query.'%', 'OR:Category.pagetitle:LIKE' => '%'.$query.'%',
            );
            $c->where($queryWhere);
        }
